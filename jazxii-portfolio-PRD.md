...
### 5.4 — Bookend Footer
A single repeated CTA card ("Let's build something inclusive together") + large wordmark/monogram footer on every route (Juan resolves Home and Work to the identical closing moment). The CTA carries `mailto:`, LinkedIn, GitHub; the footer nav stacks **Design system** / About / email **vertically** under an "Explore" kicker (uniform with the right-hand "Built with" column). The closing banner is full-bleed and fills the viewport on mobile (`min-h-dvh`). No downloads or sharing flows.

### 5.5 — Global Chrome
Persistent top nav: a left `jazxii.` wordmark (peach dot), a centred **iPhone-style "dynamic island"**, and a right cluster of `Email` / `in` / `gh` (rendered as **individual frosted glass pills**) + a live clock (12-hour) + a round glass theme toggle. The wordmark, pills, clock and toggle **adapt their tone to the section scrolling beneath the header** (see *Adaptive header chrome* below). Plus a **custom cursor**.
- **Dynamic island (`Nav.tsx`):** collapsed, it's a black pill showing a memoji album-art chip (`public/media/memoji-lap.png`) + a **looping animated equaliser**. On hover, keyboard focus, or tap it **morphs (a smooth container box-morph — width/height transitioned with `contain: layout`; trigger + panel crossfade on the compositor, no jank)** to reveal **the four nav buttons spread edge-to-edge** (`justify-content: space-between`; Home / Work / Playground / About) — the album art + EQ are hidden in the expanded view, and the active link shares the bar's `--island-radius` so it nests concentrically. The links live in a real `<nav aria-label="Main">`, always in the DOM and focusable (focus triggers `:focus-within` to expand); a trigger `<button aria-expanded aria-controls>` opens it for touch; Escape + outside-tap dismiss (WCAG 1.4.13). The EQ animates in a loop while minimised (motion-allowed) and is static under reduced motion. The island lives in the layout so it stays continuous across client-side navigation. *(A functional "now playing" music player previously lived in the expanded island; it was reverted on 2026-06-17 and archived to `archive/IslandPlayer.tsx` — original royalty-free loops remain in `public/audio/`. A richer 3D-character / GSAP-scroll treatment is planned; see `ai-sdlc.md`.)*
- **Adaptive header chrome (`Nav.tsx` + `lib/navTone.ts`, v1.4):** the header corners are **frosted glass pills** (Juan-Mora-style — `backdrop-filter: blur(11px)`, translucent fill, hairline, springy hover) so each control carries its own legible surface over any backdrop. A progressive **`.nav-blur`** band (stacked `backdrop-filter` layers, masked top→sharp) frosts content passing under the header. Crucially the chrome is **tone-adaptive**: a `header[data-tone]` palette flips between light- and dark-contents, defaulting to the page theme and **overridden per section** for regions whose tone differs from the theme — e.g. the always-dark footer banner registers `useNavTone(ref,"dark")` (via an `IntersectionObserver` at the header band), so the header stays legible (light contents) over it **even in light mode**. This fixes the prior contrast loss / invisible buttons over contrasting sections, and keeps text **AA in both directions**. The "Work" folder watermark was also switched from raw white to the theme-aware `var(--text)` so it's visible in light mode. **Responsive:** the pills + clock show on large screens only — the mobile header keeps just the island + theme toggle + wordmark — and the `.nav-blur` band is shorter/lighter on mobile so it doesn't smear high-sitting content like the footer banner's name (decisions D27 / D28 / D3c / D29).
...
- **Smooth scroll:** one global Lenis instance (`components/motion/SmoothScroll.tsx`, mounted in the layout) synced to ScrollTrigger (`lenis.on("scroll", ScrollTrigger.update)` + `gsap.ticker` drives `lenis.raf`). On every route change it resets scroll to the **top** (browser restoration off; hash deep-links exempt) and recomputes Lenis's scroll limit + `ScrollTrigger.refresh()`, so the persisted global Lenis can't strand the visitor mid-page on a taller route (decisions D21). It also recomputes the limit on **in-page content growth** — a `body` `ResizeObserver` (+ `window load` / `fonts.ready`) re-runs `lenis.resize()` + `ScrollTrigger.refresh()`, because `<html height:100%>` keeps Lenis's own observer from seeing late media/font reflow, which otherwise clamped the wheel partway down until a reload (decisions D26).
